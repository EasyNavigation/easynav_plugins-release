  ...
  double sigma, double p_min, ProgressCallback progress = nullptr)
{
  ...
  std::vector<CoordT> seeds; src->getOccupiedVoxels(seeds);
  ...
  auto write_max = [&](const CoordT & c, int32_t v){
      ...
      if (prop < clamp_min) {prop = clamp_min;}
      if (prop > clamp_max) {prop = clamp_max;}
      cell->probability_log = prop;
      cell->update_id = 0;
    };
...
std::expected<void, std::string> AMCLLocalizer::on_initialize()
{
  ...
  // New descriptive params
  node->declare_parameter<double>(plugin_name + ".inflation_stddev", 1.5);
  ...
  RCLCPP_INFO(node->get_logger(), "Init pose (%.3f, %.3f, yaw=%.3f) std_xy=%.3f std_yaw=%.3f",
...
static SensorBundle make_sensor_bundle_downsampled(
  const PointPerception & pp, double ds_vox, std::size_t cap_points)
{
  SensorBundle out;
  out.frame_id = get_frame_id_from(pp);
  const auto & in = pp.data;

  if (in.empty()) {out.cloud = pcl::PointCloud<pcl::PointXYZ>(); return out;}

  // Simple stride-based cap after a coarse voxel ds on upstream pipe if you have it.
  out.cloud.header = in.header;
  out.cloud.is_dense = in.is_dense;

  const std::size_t N = in.points.size();
  if (cap_points == 0 || N <= cap_points) {
    out.cloud = in; // copy
  } else {
    out.cloud.points.reserve(cap_points);
    const std::size_t stride = std::max<std::size_t>(1, N / cap_points);
    for (std::size_t i = 0; i < N && out.cloud.points.size() < cap_points; i += stride) {
      out.cloud.points.push_back(in.points[i]);
    }
    out.cloud.width = static_cast<uint32_t>(out.cloud.points.size());
    out.cloud.height = 1;
  }
  (void)ds_vox; // left here if you want to add a true voxel filter later
  return out;
}
  ...
  int tail_skip)
{
  using Bonxai::CoordT;
  ScoreAgg s;
  if (cloud_sensor.empty()) {return s;}

  auto acc_occ = occ_map.grid().createConstAccessor();
  s.possible = static_cast<float>(cloud_sensor.points.size());

  tf2::Transform world_T_sensor = particle_pose * T_bf_sensor;
  const tf2::Vector3 origin_w = world_T_sensor.getOrigin();
  const CoordT key_origin = occ_map.grid().posToCoord({origin_w.x(), origin_w.y(), origin_w.z()});

  std::unordered_map<CoordT, int, CoordHash, CoordEq> end_counts;
  end_counts.reserve(cloud_sensor.points.size());

  for (const auto & pt : cloud_sensor.points) {
    tf2::Vector3 pw = world_T_sensor * tf2::Vector3(pt.x, pt.y, pt.z);
    if (!std::isfinite(pw.x())) {continue;}
    const CoordT key_end = occ_map.grid().posToCoord({pw.x(), pw.y(), pw.z()});
    ++end_counts[key_end];
  }

  for (const auto & kv : end_counts) {
    const CoordT & key_end = kv.first;
    const int mult = kv.second;

    // Endpoint probability in inflated map
    float p_end = inf_map.queryProbability(key_end);

    // Gate: if very low, do not spend on ray (keep small contribution)
    constexpr float P_RAY_MIN = 0.2f;
    if (p_end < P_RAY_MIN) {
      s.hits += p_end * mult;
      continue;
    }

    // Confirm with free ray up to near-end
    const bool blocked = ray_occluded_dda(occ_map, acc_occ, key_origin, key_end, tail_skip);
    s.hits += (blocked ? 0.0f : p_end) * mult;
  }
...
void AMCLLocalizer::correct(NavState & nav_state)
{
  ...
  const double vox = inflated_map->grid().voxelSize();
  ...
  for (const auto & h : perceptions) {
    if (!h) {continue;}
    bundles.emplace_back(make_sensor_bundle_downsampled(*h, ds_vox, correct_max_points_));
  }
  ...
  for (auto & particle : particles_) {
    ...
    for (const auto & b : bundles) {
      ...
      ScoreAgg s = score_particle_sensor_cloud(
        ...
        *original_map, *inflated_map, TAIL_SKIP);
...
void AMCLLocalizer::reseed()
{
  ...
  auto safe_cov_2x2 = [](double A, double B, double C){
      const double eps = 1e-9; double a2 = A, b2 = B, c2 = C;
      if (!std::isfinite(a2) || a2 < eps) {a2 = eps;}
      if (!std::isfinite(c2) || c2 < eps) {c2 = eps;}
      if (!std::isfinite(b2)) {b2 = 0.0;}
  ...
  if (!std::isfinite(yaw_var) || yaw_var < 0.0) {yaw_var = 0.0;}
  double yaw_std = std::sqrt(yaw_var); if (!std::isfinite(yaw_std) || yaw_std < min_noise_yaw_) {
    yaw_std = min_noise_yaw_;
  }

  std::normal_distribution<double> yaw_noise(0.0, yaw_std);
  ...
  auto sample_xy_offset = [&]() -> std::pair<double, double> {
      ...
      double dx = l00 * z0, dy = l10 * z0 + l11 * z1;
      ...
      if (!(r >= min_noise_xy_)) {
        if (r < 1e-12) {
          std::normal_distribution<double> nIso(0.0, min_noise_xy_); dx = nIso(rng_);
          dy = nIso(rng_);
        } else {double s = (min_noise_xy_ / r); dx *= s; dy *= s;}
      }
      return {dx, dy};
    };

  for (std::size_t i = N_top; i < N; ++i) {
    int selected_idx; int guard = 0;
    do {
    ...
    double rr, pp, yy; tf2::Matrix3x3(ref.pose.getRotation()).getRPY(rr, pp, yy);
    double noisy_yaw = yy + yaw_noise(rng_); if (!std::isfinite(noisy_yaw)) {noisy_yaw = yy;}
    tf2::Quaternion q; q.setRPY(0.0, 0.0, noisy_yaw);
    if (q.length2() < 1e-20 || !std::isfinite(q.x()) || !std::isfinite(q.y()) ||
      !std::isfinite(q.z()) || !std::isfinite(q.w())) {q.setRPY(0.0, 0.0, yy);}
  ...
  double tw = 0.0; for (const auto & p : particles_) {
    if (std::isfinite(p.weight)) {
      tw += p.weight;
    }
  }
  if (tw > 0.0 && std::isfinite(tw)) {
    for (auto & p : particles_) {
      p.weight /= tw; if (!std::isfinite(p.weight) || p.weight < 0.0) {
        p.weight = 0.0;
      }
    }
  } else {
