  ...
  ProgressCallback progress /*= nullptr*/)
{
  ...
  write_max(CoordT{0, 0, 0}, std::numeric_limits<int32_t>::min()); // no-op
  for (const auto & c : seeds) {
...
// Devuelve true si hay ocupado ANTES del final (dejando tail voxels sin comprobar)
  ...
  int tail_skip_voxels = 1)
{
  ...
  // Errores acumulados (enteros); inicializados a la mitad del máximo para simetría
  int ex = (dx >= dy && dx >= dz) ? dx / 2 : (dy >= dz ? dy / 2 : dz / 2);
  int ey = ex, ez = ex;

  for (int step = 0; step < nsteps; ++step) {
    // deja sin comprobar los últimos 'tail_skip_voxels' y el final
...
AMCLLocalizer::correct(NavState & nav_state)
{
  ...
  double stdev_inflation = 1.5; // metros; expón como parámetro si quieres
  double p_min = 0.1; // metros; expón como parámetro si quieres
  ...
  if (!nav_state.has("map.bonxai.inflated")) {
    ...
    std::shared_ptr<Bonxai::ProbabilisticMap> inflated_map =
      inflate_map(original_map, stdev_inflation, p_min, progress_cb);
  ...
  // === Downsample adaptado al tamaño de vóxel del mapa inflado + límite duro de puntos
  const double vox = map->grid().voxelSize();
  const double ds_vox = std::max(0.5, 2.0 * vox);   // 2x voxel ~ agresivo y barato
  auto view = PointPerceptionsOpsView(perceptions)
    .downsample(ds_vox)
    .fuse(get_tf_prefix() + "base_footprint");
  auto filtered = view->as_points();

  static constexpr size_t MAX_PTS = 1500;
  if (filtered.size() > MAX_PTS) {
    pcl::PointCloud<pcl::PointXYZ> slim;
  // Copiamos metadatos razonables
    slim.header = filtered.header;
    slim.is_dense = filtered.is_dense;
    slim.points.reserve(MAX_PTS);

    const size_t stride = std::max<size_t>(1, filtered.size() / MAX_PTS);
    for (size_t i = 0; i < filtered.size() && slim.points.size() < MAX_PTS; i += stride) {
    ...
    filtered.swap(slim); // ahora sí: ambos son pcl::PointCloud<pcl::PointXYZ>
  }
  ...
  // === Parámetros de gating y tolerancia
  constexpr float P_RAY_MIN = 0.2f;   // Sólo raytrazar si el endpoint parece consistente


  const double res_vox = original_map->grid().voxelSize();
  const double d_allow = p_min * std::sqrt(-2.0 * std::log(stdev_inflation));
  const int    TAIL_SKIP = std::max(1, static_cast<int>(std::ceil(d_allow / res_vox)));

  for (auto & particle : particles_) {
    float hits = 0.0f;
    float possible_hits = 0.0f;

    // Accessors por partícula (lectura): baratos y evitan recreate por consulta
    auto acc_occ = original_map->grid().createConstAccessor();
    auto acc_inf = map->grid().createConstAccessor();

    // Origen del rayo (una vez por partícula)
    ...
    // Agrupa endpoints por vóxel final para no repetir trabajo
    ...
    for (const auto & pt : filtered) {
      ...
      const Bonxai::CoordT key_end =
        original_map->grid().posToCoord({pw.x(), pw.y(), pw.z()});
      ++end_counts[key_end];
    }
    ...
    for (const auto & kv : end_counts) {
      ...
      // Probabilidad del endpoint (rápido, sin crear accessors)
      float p_end = 0.0f;
      if (const auto * cell_end = acc_inf.value(key_end)) {
        // Si tienes un helper estático ProbabilisticMap::prob(int32_t), úsalo:
        p_end = Bonxai::ProbabilisticMap::prob(cell_end->probability_log);
        // Si no existiera, descomenta:
        // p_end = map->queryProbability(key_end);
      }

      if (p_end < P_RAY_MIN) {
        hits += p_end * mult;    // contribución baja -> no raytrazamos
        continue;
      }

      // Pinta bien -> confirmar ausencia de oclusión en mapa original
  ...
  // === Pesos (potencia tau) + normalización
